...
export class NegociacionComponent implements OnInit {
  copys = [
    'Propongo acuerdos entre varios puntos de vista buscando que todos logremos obtener ganancias.',
  ...
  seleccionar(event, opt) {
    this.select = opt;
    this.rptasNegociacion[this.index] = parseFloat(event.srcElement.value);
